...
// How the money actually works — read by every analysis/digest/question call so
// the model stops treating long rentals as a problem (v556, Jake's correction).
const ECONOMICS = "How the money works — apply this before judging any number: (1) Bin rentals bill a flat rate for an included period plus a PER-EXTRA-DAY charge afterwards, so bins out on long rentals are EXTRA REVENUE — high avg rental days and bins out 14+ days ('overdue_bins') are good news, not a problem, unless so many bins are out that new bookings get turned away. A bin in a driveway is also a street-level billboard. Never recommend chasing bins back just because they've been out long. (2) Dollar amounts live in QuickBooks, not in this data — never invent or estimate dollar figures; reason in job counts and directions instead. (3) this_month.last_year is the FULL calendar month last year — when judging the current month mid-month, pro-rate by day of month before comparing, and say you did. (4) Fast bin turnaround only matters when a size sells out; spare capacity means longer rentals are pure upside.";
