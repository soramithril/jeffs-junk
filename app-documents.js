...
var DOCUMENTS = [
  ...
  { name: 'Typed Quote',                 group: 'Junk & Donations', desc: 'Fill in on the computer and email. For bigger jobs that need a written quote with a total.', file: 'Jeffs Junk - Typed Quote 2026.docx' },
...
function docKind(file){
  return /\.docx$/i.test(file) ? 'Word · downloads, fill it in on your computer'
...
function docCard(d){
  ...
  return '<a class="chart-card" href="'+href+'" target="_blank" rel="noopener" '
    ...
    + '<div style="min-width:0">'
      ...
      + '<div style="font-size:11px;color:var(--muted);margin-top:6px">' + docKind(d.file) + '</div>'
    + '</div>'
  + '</a>';
}
