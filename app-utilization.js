...
async function renderUtilization(){
  ...
  document.getElementById('util-metrics').innerHTML=
    '<div class="metric-box"><div class="metric-val" id="utm-util" style="--kc:var(--accent)">0%</div><div class="metric-lbl">Overall Utilization (90d)</div><div style="font-size:10px;color:var(--muted);margin-top:3px">share of fleet time actually rented</div></div>'
    ...
    +'<div class="metric-box"><div class="metric-val" id="utm-idle" style="--kc:'+(idleSizeCount>0?'var(--bad)':'var(--accent)')+'">0</div><div class="metric-lbl">Idle Sizes (30d)</div><div style="font-size:10px;color:var(--muted);margin-top:3px">sizes with no rentals in 30 days</div></div>'
    +'<div class="metric-box" style="grid-column:1/-1"><div style="display:flex;justify-content:center;gap:32px"><div style="text-align:center"><div class="metric-val" style="--kc:var(--bad)">'+currentOut+'</div><div class="metric-lbl">Currently Out</div></div><div style="text-align:center"><div class="metric-val" style="--kc:var(--accent)">'+currentIn+'</div><div class="metric-lbl">Currently In Yard</div></div><div style="text-align:center"><div class="metric-val">'+totalBinCount+'</div><div class="metric-lbl">Total Fleet</div></div></div><div style="height:8px;background:var(--surface2);border-radius:4px;margin:12px 0"><div style="height:100%;background:var(--bad);border-radius:4px;width:'+Math.round(totalBinCount?currentOut/totalBinCount*100:0)+'%"></div></div><div style="font-size:12px;color:var(--muted);text-align:center">'+Math.round(totalBinCount?currentOut/totalBinCount*100:0)+'% of fleet currently deployed</div></div>';
  ...
  document.getElementById('util-by-city').innerHTML=makeBarChart(cityData,function(){return 'var(--accent)';},function(v){return v+' days';});
  ...
  if(!idleBins.length){
    ...
    idleEl.innerHTML='<div style="font-size:13px;color:var(--muted);margin-bottom:10px">These bin sizes had no rentals in the last 30 days. Consider promotions or price adjustments.</div>'
      +Object.keys(idleBySize).map(function(sz){return'<div style="padding:8px 12px;border:1px solid rgba(220,38,38,.3);border-radius:8px;margin-bottom:6px;background:rgba(220,38,38,.05);display:flex;justify-content:space-between"><strong>'+sz+'</strong><span style="color:var(--red);font-weight:700">no rentals in 30 days · '+idleBySize[sz]+' bins in fleet</span></div>';}).join('');
  }
